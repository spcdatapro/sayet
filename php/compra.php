...
$app->post('/c', function(){
    ...
    $query.= "idtipofactura, isr, idtipocombustible, galones, idp, idproyecto, idunidad, nombrerecibo, alcontado, idusuario) ";
    ...
    $query.= $d->idtipocombustible.", ".$d->galones.", ".$d->idp.", $d->idproyecto, $d->idunidad, $d->nombrerecibo, $d->alcontado, $d->idusuario";
    $query.= ")";
    $db->doQuery($query);
...
$app->post('/u', function(){
    ...
    $query.= "nombrerecibo = $d->nombrerecibo, alcontado = $d->alcontado, retiva = $d->retIva, ultusuario = $d->idusuario ";
    $query.= "WHERE id = ".$d->id;
    $db->doQuery($query);
...
$app->get('/uisr/:idcompra/:monto/:idempresa/:suma/:tc', function ($idcompra, $monto, $idempresa, $suma, $tc) {
    $db = new dbcpm();
    ...
    if ($suma) {
        $monto_prov = $db->getOneField("SELECT ROUND(haber + 0.01 * $tc, 2) FROM detallecontable WHERE idorigen = $idcompra AND origen = 2 AND idcuenta = $ctaprov");
    } else {
        $monto_prov = $db->getOneField("SELECT ROUND(haber - 0.01 * $tc, 2) FROM detallecontable WHERE idorigen = $idcompra AND origen = 2 AND idcuenta = $ctaprov");
    }

    $db->doQuery("UPDATE compra SET isr = $monto WHERE id = $idcompra");
    $db->doQuery("UPDATE detallecontable SET haber = $monto * $tc WHERE idorigen = $idcompra AND origen = 2 AND idcuenta = $ctaisr");
...
$app->get('/uriva/:idcompra/:monto/:idempresa/:suma/:tc', function ($idcompra, $monto, $idempresa, $suma, $tc) {
    $db = new dbcpm();
