...
$app->post('/prestamos', function(){
    ...
    $query.= "SELECT 
                ...
                0 AS descnomina,
                j.monto AS descuento,
                IF(DATE_FORMAT(g.fecha, '%Y-%m') = DATE_FORMAT('$fecha_inicio', '%Y-%m'), 0, 0) + IFNULL(j.monto, 0.00) AS totdesc,
                (g.monto - COALESCE((SELECT 
                                SUM(pn.monto)
                            FROM
                                plnpresnom pn
            ...
            FROM
                ...
                plnpuesto d ON a.idplnpuesto = d.id
                    INNER JOIN
                plnprestamo g ON a.id = g.idplnempleado
                    INNER JOIN
                plnempresa h ON c.idempresadebito = h.id
                    INNER JOIN
                proyecto i ON c.idproyecto = i.id
                    LEFT JOIN
                (SELECT 
            ...
            WHERE
                g.anulado = 0 AND g.esembargo = 0
                    ...
                    AND (DATE_FORMAT(g.fecha, '%Y-%m') = DATE_FORMAT('$fecha_inicio', '%Y-%m') OR DATE_FORMAT(g.liquidacion, '%Y-%m') = DATE_FORMAT('$fecha_inicio', '%Y-%m')) ";
    $query.= isset($d->idempresa) ? "AND h.id = $d->idempresa " : "";
