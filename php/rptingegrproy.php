...
$app->post('/detalle', function () use ($db) {
    ...
    $query .= "FROM factura a INNER JOIN detfact b ON a.id = b.idfactura INNER JOIN contrato c ON c.id = a.idcontrato ";
    $query .= "INNER JOIN tiposervicioventa d ON d.id = b.idtiposervicio ";
    $query .= "WHERE a.anulada = 0 AND MONTH(a.fecha) = $d->mes AND YEAR(a.fecha) = $d->anio AND c.idproyecto = $d->idproyecto ";
    $query .= "AND IF($d->idproyecto = 2 OR $d->idproyecto = 16, b.idtiposervicio NOT IN(1), b.idtiposervicio NOT IN(1, 16)) AND a.idempresa = $d->idempresa ";
    $query .= "ORDER BY 1";
    $conceptos = $db->getQuery($query);
    $cntConceptos = count($conceptos);
    $totIngresos = 0.00;
    ...
    if ($antesAbrPla) {
        ...
        if (count($datosPlanilla) > 0 || count($datosPlanillaEspecial) > 0) {
            ...
            $datos->egresos[] = [
                'concepto' => 'PLANILLA',
                'monto' => (float) $totPlanilla,
                'detalle' => $detPln
            ];
            $totEgresos += (float) $totPlanilla;
        }
    }
    //Fin de Egresos de planilla
...
function getQueryDepreciaciones($idempresa, $idproyecto, $mes, $anio, $sinDetalle, $soloDepre)
{
