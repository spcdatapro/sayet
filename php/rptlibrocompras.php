...
$app->post('/rptlibcomp', function(){
    ...
    $query.= "IF(a.idtipocompra = 3, IF(a.idtipofactura <> 7, round(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1),2), 0.00), 0.00) + ";
    $query.= "IF(a.idtipocompra IN(1, 4), IF(a.idtipofactura <> 7, round(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1),2), 0.00), 0.00) AS bien, ";
    $query.= "IF(a.idtipocompra = 2, IF(a.idtipofactura <> 7, round(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1),2), 0.00), 0.00) AS servicio, ";
    $query.= "IF(a.idtipocompra = 1, IF(c.generaiva = 0 AND a.idtipofactura <> 7, ROUND(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00) + ";
    $query.= "IF(a.idtipocompra IN(2, 3), IF(c.generaiva = 0 AND a.idtipofactura NOT IN(3, 7), ROUND(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00)+a.idp+a.noafecto AS exento, ";
    $query.= "IF(a.idtipocompra <> 5, IF(c.generaiva = 1 AND a.idtipofactura = 7, ROUND((a.subtotal - a.noafecto) * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00) AS importaciones, ";
    ...
    $query.= "IF(a.idtipocompra = 3, IF(c.generaiva = 1 AND a.idtipofactura <> 7, round(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1),2), 0.00), 0.00) + ";
    $query.= "IF(a.idtipocompra IN(1, 4), IF(c.generaiva = 1 AND a.idtipofactura <> 7, ROUND(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00) AS bien, ";
    $query.= "IF(a.idtipocompra = 2, IF(c.generaiva = 1 AND a.idtipofactura <> 7, ROUND(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00) AS servicio, ";
    $query.= "IF(a.idtipocompra = 1, IF(c.generaiva = 0 AND a.idtipofactura <> 7, ROUND(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00) + ";
    $query.= "IF(a.idtipocompra IN(2, 3), IF(c.generaiva = 0 AND a.idtipofactura NOT IN(3, 7), ROUND(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00)+a.idp+a.noafecto AS exento, ";
    $query.= "IF(a.idtipocompra <> 5, IF(c.generaiva = 1 AND a.idtipofactura = 7, ROUND(a.subtotal * IF(a.idmoneda = 2, a.tipocambio, 1), 2), 0.00), 0.00) AS importaciones, ";
...
$app->get('/detgastact/:idempresa/:mes/:anio', function($idempresa, $mes, $anio){
	...
	for($i = 0; $i < 2; $i++){
		...
		if(count($compras) > 0){
			...
			$query.= "FROM detallecontable a INNER JOIN cuentac b ON b.id = a.idcuenta INNER JOIN compra c ON c.id = a.idorigen LEFT JOIN proveedor d ON d.id = c.idproveedor ";
			$query.= "WHERE a.origen = 2 AND a.anulado = 0 AND c.idempresa = $idempresa AND c.mesiva = $mes AND YEAR(c.fechaingreso) = $anio AND b.codigo LIKE '$cuenta%' ";
			$query.= "AND (c.idtipofactura != 5 OR d.pequeniocont = 0) ";
			$query.= "ORDER BY c.fechaingreso";
