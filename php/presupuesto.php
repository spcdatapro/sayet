    ...
    for ($i = 0; $i < $cntPagos; $i++) {
        ...
        if (count($detspago) > 0) {
            ...
            if ((int)$lastid > 0) {
                ...
                $url = 'http://localhost/php/tranbanc.php/doinsdetcont'; //Producción
                $obj->anticipo = 1;
...
$app->post('/prtaprobacion', function() {
    ...
    $query = "SELECT 
                ...
                DATE_FORMAT(DATE_ADD(NOW(),
                            INTERVAL IFNULL(d.diascred, 7) DAY),
                        '%d/%m/%Y') AS fpago,
    ...
    $datos->avance = round(($afectar * 100) / $datos->monto, 2); 
...
$app->get('/monto/:ot/:correla', function($ot, $correla) {
    $db = new dbcpm();
    $ids = array();
    $ots = array();
