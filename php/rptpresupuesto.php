...
function getDocumentosOT($db, $idot){
    ...
    for($i = 0; $i < $cntDocs; $i++){
        $doc = $documentos[$i];
        $query = "
            SELECT idtranban, GROUP_CONCAT(DISTINCT documento SEPARATOR ', ') AS documento, SUM(totfact) AS totfact, SUM(isr) AS isr
            FROM(
            ...
            WHERE z.idtranban = $doc->id
            UNION
            SELECT z.idtranban, GROUP_CONCAT(CONCAT(y.serie, y.documento) SEPARATOR ', ') AS documento, SUM(IF(x.eslocal = 1, y.totfact, y.totfact * y.tipocambio)) AS totfact, SUM(IF(x.eslocal = 1, y.isr, y.isr * y.tipocambio)) AS isr 
            FROM doctotranban z
            INNER JOIN compra y ON y.id = z.iddocto
            INNER JOIN moneda x ON x.id = y.idmoneda
            WHERE z.idtranban = $doc->id AND z.idtipodoc = 1 AND z.iddocto NOT IN(SELECT idcompra FROM detpagocompra WHERE idtranban = $doc->id)
            UNION
            ...
            WHERE z.idtranban = $doc->id AND z.idtipodoc = 2
...
$app->post('/rptpresupuesto', function(){
    ...
    $query.= "FORMAT(totalpagosprogramados, 2) AS totalpagosprogramados, FORMAT(montoavance, 2) AS montoavance, notas, usrcrea, DATE_FORMAT(fechacreacion, '%d/%m/%Y') AS fechacreacion, ";
    ...
    $query.= "CONCAT(FORMAT(IF(totalpresupuesto > totalpagosprogramados, montoavance * 100 / totalpresupuesto, montoavance * 100 / totalpagosprogramados), 2), '%') AS avancegeneral ";
    ...
    $query.= "FORMAT(pagosprogramados, 2) AS pagosprogramados, FORMAT(montoavance, 2) AS montoavance, notas, ";
    $query.= "FORMAT(IF(monto > pagosprogramados, monto, pagosprogramados), 2) AS montoreal, CONCAT(FORMAT(IF(monto > pagosprogramados, montoavance * 100 / monto, montoavance * 100 / pagosprogramados), 2), '%') AS avanceot ";
