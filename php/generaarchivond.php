...
$app->post('/generar', function() use($db){
    ...
    $query = "SELECT z.tipo, z.cuenta, LPAD(@row := @row + 1, 20, ' ') AS contador, z.nombre, z.monto, limpiaString(z.concepto) AS concepto, z.bancopais
    FROM (
        ...
        IF(b.idbancopais = c.idbancopais, null, d.nombre) AS bancopais
        FROM tranban a
        INNER JOIN proveedor b ON b.id = a.idbeneficiario
		INNER JOIN banco c ON c.id = a.idbanco
        LEFT JOIN bancopais d ON d.id = c.idbancopais
        WHERE a.tipotrans = 'B' AND a.anulado = 0 AND a.esplanilla = 0 AND a.origenbene AND a.fecha = '$d->fechastr' AND a.idbanco = $d->idbanco 
        UNION
        SELECT 1 AS tipo, LPAD(TRIM(IFNULL(b.cuentabanco, ' ')), 10, ' ') AS cuenta, RPAD(TRIM(limpiaString(a.beneficiario)), 100, ' ') AS nombre, LPAD(a.monto, 25, ' ') AS monto, TRIM(a.concepto) AS concepto, 
        IF(b.idbancopais = c.idbancopais, null, d.nombre) AS bancopais
        FROM tranban a
