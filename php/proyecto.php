...
$app->get('/lstproyectoporempresa/:idempresa/:iusuario', function($idempresa, $idusuario){
    ...
    $query.= "FROM proyecto a INNER JOIN empresa b ON b.id = a.idempresa INNER JOIN tipo_proyecto c ON c.id = a.tipo_proyecto ";
    $query.= $idusuario > 0 ? "INNER JOIN usuarioproyecto d ON d.idproyecto = a.id " : "";
    $query.= "WHERE (a.idempresa = $idempresa OR a.id IN (SELECT idproyecto FROM empresa_proyecto WHERE idempresa = $idempresa)) ";
...
$app->post('/c', function(){
    ...
    $query.= ", ".$d->metros_rentable.", ".$d->tipo_proyecto.", ".$d->subarrendado.", NULL, '".$d->referencia."', $d->fechaaperturastr, $d->multiempresa, '$d->otras_empresas')";
    $db->doQuery($query);
...
$app->post('/csu', function(){
    $d = json_decode(file_get_contents('php://input'));
    $db = new dbcpm();
    if (!isset($d->idcontrato)) { $d->idcontrato = 'null'; };
