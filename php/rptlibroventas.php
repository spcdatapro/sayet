...
$app->post('/rptlibventas', function(){
    ...
    $query.= "IF(a.anulada = 0, IF(a.idtipoventa = 4, IF(c.generaiva = 1 AND a.idtipofactura <> 6 AND a.importeexento = 0, ROUND((a.total - a.noafecto - a.iva), 2), 0.00), 0.00), 0.00) AS activo, ";
    $query.= "IF(a.anulada = 0, IF(a.idtipoventa = 1, IF(c.generaiva = 1 AND a.idtipofactura <> 6 AND a.importeexento = 0, ROUND((a.total - a.noafecto - a.iva), 2), 0.00), 0.00), 0.00) AS bien, ";    	
	$query.= "IF(a.anulada = 0, IF(a.idtipoventa = 2, IF(c.generaiva = 1 AND a.idtipofactura <> 6 AND a.importeexento = 0, ROUND(a.subtotal - a.iva, 2), 0.00), 0.00), 0.00) AS servicio, ";	
