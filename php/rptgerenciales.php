            ...
            FROM
                ...
                (SELECT 
                    b.id,
                        ...
                        SUM(b.subtotal) - SUM((SELECT SUM(IF(dc.idrecibocli != a.idrecibocli AND dc.fecha = '$d->fechastr', dc.monto, 0)) FROM detcobroventa dc INNER JOIN recibocli rc ON dc.idrecibocli = rc.id WHERE dc.idfactura = b.id)) AS ingreso,
                        SUM(b.retisr) AS isr,
                        SUM(b.retiva) AS iva,
                        SUM(IF(b.subtotalcnv > 0, b.subtotalcnv, b.subtotal / b.tipocambio)) - SUM((SELECT SUM(IF(dc.idrecibocli != a.idrecibocli AND dc.fecha = '$d->fechastr', dc.monto, 0)) FROM detcobroventa dc INNER JOIN recibocli rc ON dc.idrecibocli = rc.id WHERE dc.idfactura = b.id)) AS ingresodlr,
            ...
            foreach ($facturas as $factura) {
                ...
                if (!isset($saldoFacturas[$factura])) {
                    ...
                    if ($countFacturas[$factura] == $index[$factura]) {
                        ...
                        $row->isr = 0;
                        $row->diferencia =  ($row->ingreso - ($row->deposito + $row->iva)) * -1;
                    } else {
                        $row->ingreso = $saldoFacturas[$factura];
