...
$app->post('/control_ingresos', function () {
    ...
    $query = "SELECT 
                ...
                ROUND(IFNULL(SUM(d.ingreso), 0), 2) AS ingreso,
                a.monto AS deposito,
                ROUND(IFNULL(SUM(d.isr), 0), 2) AS isr,
                ROUND(IFNULL(SUM(d.iva), 0), 2) AS iva,
                (ROUND(IFNULL(SUM(d.ingreso), 0), 2) - a.monto - ROUND(IFNULL(SUM(d.isr), 0), 2) - ROUND(IFNULL(SUM(d.iva), 0), 2)) * IF(d.ingreso IS NULL, 1, - 1) AS diferencia,
                e.simbolo AS moneda
            FROM
                tranban a
                    INNER JOIN
                ...
                (SELECT 
                    a.idrecibocli,
                        ...
                        SUM(IF(b.pagada = 1, IF(b.idmonedafact = 2, b.subtotalcnv, (a.monto + b.retisr + b.retiva) / b.tipocambio), IF(b.idmonedafact = 2, b.subtotalcnv, b.subtotal / b.tipocambio))) AS ingreso,
                        SUM(IF(b.idmonedafact = 2, b.retisrcnv, b.retisr / b.tipocambio)) AS isr,
                        SUM(IF(b.idmonedafact = 2, b.retivacnv, b.retiva / b.tipocambio)) AS iva,
...
$app->post('/ocupacion', function() {
    ...
    for ($y = $d->anio_del; $y <= $d->anio_al; $y++) {
        ...
        $yearObj->columnas = contarMeses($d->mes_del, $d->mes_al) + 1;
