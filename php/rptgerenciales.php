        ...
        for ($i = 0; $i < $cntCuentas; $i++) {
            ...
            if ($cuenta->id > 0) {
                $query = "SELECT 
                            ...
                            SUBSTRING(REPLACE(e.beneficiario, 'Ñ', 'N'), 1, 17) AS beneficiario,
                            IFNULL(CONCAT(f.idpresupuesto, '-', f.correlativo),
                                    '') AS orden,
                            SUBSTRING(b.conceptomayor, 1, 20) AS concepto,
                        ...
                        UNION SELECT 
                            ...
                            SUBSTRING(IFNULL(SUBSTRING(REPLACE(e.beneficiario, 'Ñ', 'N'), 1, 17), SUBSTRING(REPLACE(g.beneficiario, 'Ñ', 'N'), 1, 17)), 1, 17) AS beneficiario,
                            IFNULL(CONCAT(f.idpresupuesto, '-', f.correlativo),
                                    '') AS orden,
                            SUBSTRING(b.conceptomayor, 1, 20) AS concepto,
                            DATE_FORMAT(b.fechafactura, '%d/%m/%Y') AS fechafact,
                    ...
                    $query = "SELECT 
                                ...
                                NULL AS cheque,
                                SUBSTRING(CONCAT(b.nombre, ' ', IFNULL(b.apellidos, '')), 1, 17) AS beneficiario,
                                NULL AS orden,
                                NULL AS concepto,
                                NULL AS fechafact,
