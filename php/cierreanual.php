...
$app->post('/cierre', function () {
    ...
    $origenes = ['tranban' => 1, 'compra' => 2, 'venta' => 3, 'directa' => 4, 'reembolso' => 5, 'recint' => 12];
    ...
    for ($i = 0; $i < $cntPasCap; $i++) {
        $pc = $pascap[$i];
        $cierre->balances[] = [
            'idcuentac' => $pc->idcuentac, 'codigo' => $pc->codigo, 'nombrecta' => $pc->nombrecta,
...
function getSelectBalances($cual, $d, $ini)
{
    ...
    switch ($cual) {
        ...
        case 12:
            ...
            $query .= "WHERE a.origen = 12 AND a.activada = 1 AND a.anulado = 0 AND b.fecha <= '$d->falstr' AND b.idempresa = " . $d->idempresa . " AND c.codigo LIKE '" . $ini . "%' ";
            $query .= "GROUP BY a.idcuenta ORDER BY a.idcuenta";
            break;
    }
    return $query;
