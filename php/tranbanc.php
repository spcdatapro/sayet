...
$app->post('/c', function () {
    ...
    if (in_array($d->tipotrans, $ttsalida)) {
        if ($d->tipotrans === 'C') {
            $db->doQuery("UPDATE banco SET correlativo = correlativo + 1 WHERE id = " . $d->idbanco);
        }
...
$app->post('/d', function () {
    ...
    if (trim($tran->tipotrans) == 'C') {
        $db->doQuery("UPDATE banco SET correlativo = $tran->numero WHERE id = $tran->idbanco");
    }
...
function getInfoCheque($db, $idtran, $idusr)
{
    ...
    $camposdetcont = [];
    if (count($detcont) > 0) {
        foreach ($detcont[0] as $key => $value) {
            $camposdetcont[] = $key;
        }
    }
...
// Conciliación bancaria automática. JAragon
