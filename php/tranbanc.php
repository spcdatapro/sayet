...
$app->get('/lstbeneficiarios', function(){
    $db = new dbcpm();
    $query = "SELECT id, CONCAT(nombre, ' (', nit, ')') AS beneficiario, chequesa, 1 AS dedonde, concepto, CONVERT(retensionisr, UNSIGNED) AS retieneisr, 'Proveedor(es)' AS grupo FROM proveedor UNION ";
    $query.= "SELECT id, CONCAT(nombre, ' (', nit, ')') AS beneficiario, nombre AS chequesa, 2 AS dedonde, concepto, 0 AS retieneisr, 'Beneficiario(s)' AS grupo FROM beneficiario ";
    $query.= "ORDER BY 4, 2";
    print $db->doSelectASJson($query);
});
