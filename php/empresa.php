...
$app->get('/lstempresas', function(){
    ...
    $query.= "a.ndplanilla, a.retenedora, a.telefono ";
...
$app->post('/c', function(){
    ...
    $query = "INSERT INTO empresa(nomempresa, idmoneda, propia, abreviatura, nit, seriefact, correlafact, fechavencefact, ultimocorrelativofact, direccion, ndplanilla, retenedora, telefono) VALUES(";
    $query.= "'$d->nomempresa', $d->idmoneda, $d->propia, '$d->abreviatura', '$d->nit', $d->seriefact, $d->correlafact, $d->fechavencefactstr, $d->ultimocorrelativofact, '$d->direccion', $d->ndplanilla, $d->retenedora, '$d->telefono'";
    $query.= ")";
    $db->doQuery($query);
...
$app->post('/u', function(){
    ...
    $query.= "fechavencefact = $d->fechavencefactstr, ultimocorrelativofact = $d->ultimocorrelativofact, direccion = '$d->direccion', ndplanilla = $d->ndplanilla, retenedora = $d->retenedora, telefono = '$d->telefono' ";
    $query.= "WHERE id = $d->id";
    $db->doQuery($query);
});
