...
$app->get('/srchcli/:idempresa/:qstra+', function($idempresa, $qstra){
    ...
    $query.= "FROM factura ";
    $query.= "WHERE idempresa = $idempresa AND nombre LIKE '%$qstr%' AND (idcontrato = 0 OR idcontrato IS NULL) AND TRIM(nit) NOT IN(SELECT TRIM(nit) FROM detclientefact WHERE fal IS NULL AND facturara LIKE '%$qstr%') ";
    $query.= "ORDER BY 2";
...
$app->post('/cd', function(){
    ...
    $query.= ") VALUES(";
    $query.= "$d->idfactura, $d->cantidad, $d->idtiposervicio, $d->mes, $d->anio, '$d->descripcion', ".((float)$d->preciounitario - (float)$d->descuento).", ".((float)$d->preciotot - (float)$d->descuento).", $d->descuento, $d->preciotot, $d->preciotot";
    $query.= ")";
    $db->doQuery($query);
    $lastid = $db->getLastId();
    updateDatosFactura($d);
