(function(){
    ...
    tranaprobpresupctrl.controller('tranAprobAmpCtrl', ['$scope', 'presupuestoSrvc', '$confirm', '$filter', 'authSrvc', 'DTOptionsBuilder', '$uibModal', 'toaster', ($scope, presupuestoSrvc, $confirm, $filter, authSrvc, DTOptionsBuilder, $uibModal, toaster) => {
        ...
        $scope.getPendientes = () => presupuestoSrvc.ampliacionPresup($scope.usrdata.uid).then((d) => { $scope.presupuestos = procData(d); });
            ...
            if(+obj.aprobada === 1){
                $confirm({text: '¿Esta seguro(a) de aprobar la ampliación No. ' + obj.ampliacion +'?', title: 'Aprobar ampliación', ok: 'Sí', cancel: 'No'}).then(function() {
                    ...
                    presupuestoSrvc.editRow({ idamplia: obj.id }, 'apamp').then(function(){
                        $scope.getPendientes();
                        toaster.pop('info', 'Ampliación aprobado', 'Se aprobó la ampliación No. ' + obj.ampliacion, 'timeout:1500');
                    });
                }, function(){
                    obj.aprobada = 0;
                });
            ...
            if(+obj.denegada === 1){
                $confirm({text: '¿Esta seguro(a) de denegar el ampliación No. ' + obj.ampliacion +'?', title: 'Denegar ampliación', ok: 'Sí', cancel: 'No'}).then(function() {
                    ...
                    presupuestoSrvc.editRow({ idamplia: obj.id }, 'rechamp').then(function(){
                        $scope.getPendientes();
                        toaster.pop('info', 'Ampliación denegado', 'Se denegó el ampliación No. ' + obj.ampliacion, 'timeout:1500');
                    });
                }, function(){
                    obj.denegada = 0;
                });
