(function(){
    ...
    genpagosplnctrl.controller('genPagosPlnCtrl', ['$scope', '$window', 'authSrvc', 'empresaSrvc', '$filter', 'planillaSrvc', 'jsReportSrvc', '$uibModal', function ($scope, $window, authSrvc, empresaSrvc, $filter, planillaSrvc, jsReportSrvc, $uibModal) {
        ...
        $scope.genND = function () {
            ...
            planillaSrvc.empresas($scope.params).then(function (d) {
                ...
                }).result.then(function () { }, function () { return 0; });
            });
        };
    ...
    //------------------------------------------------------------------------------------------------------------------------------------------------//
    genpagosplnctrl.controller('modalDatosND', ['$scope', '$uibModalInstance', 'empresas', 'params', '$confirm', 'jsReportSrvc', '$window', 'tranBancSrvc', 'planillaSrvc', 'toaster', function ($scope, $uibModalInstance, empresas, params, $confirm, jsReportSrvc, $window, tranBancSrvc, planillaSrvc, toaster) {
        ...
        $scope.empresas.forEach(empresa => {
            console.log(empresa);
            if (empresa.existe != 0) {
                empresa.ndplanilla = empresa.existe.numero;
                empresa.idbanco = empresa.existe.idbanco;
            }
        });

        $scope.revertirND = (idtranban) => {
            $confirm({ text: 'Esto revertirá la nota de débito seleccionada y liberara la planilla. ¿Seguro(a) de continuar?', title: 'Revertir nota de débito', ok: 'Sí', cancel: 'No' }).then(function () {
                tranBancSrvc.revertir(idtranban).then(function (d) {
                    toaster.pop({
                        type: d.tipo,
