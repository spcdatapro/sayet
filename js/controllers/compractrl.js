(function () {
                ...
                if (angular.isDate(fecha)) {
                    if (fecha.getFullYear() >= 2000) {
                        ...
                        periodoIvaSrvc.validaFecha(fecha).then(d => {
                            ...
                            $scope.periodoCerrado = +periodoIva === 0 && $scope.laCompra.totfact > 0 && !$scope.laCompra.id ? $scope.calcular() : false;
                        });
                    } else {
                        $scope.periodoCerrado = true;
                    }
    ...
    compractrl.controller('ModalIVA', ['$scope', '$uibModalInstance', 'compra', 'compraSrvc', function (
        ...
        $scope.ok = function () {
            ...
            compraSrvc.editRow($scope.compra, 'civa').then(function () { $uibModalInstance.close(compra.id); });
        };
