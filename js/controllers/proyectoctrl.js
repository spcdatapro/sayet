(function () {
    ...
    proyectoctrl.controller('proyectoCtrl', ['$scope', 'proyectoSrvc', 'empresaSrvc', 'tipoProyectoSrvc', 'proyectoAdjuntoSrvc', 'tipoAdjuntoSrvc', 'Upload', 'activoSrvc', 'proyectoActivoSrvc', '$confirm', 'tipoDocProySrvc', 'unidadSrvc', 'tipoLocalSrvc', 'DTOptionsBuilder', '$filter', 'servicioBasicoSrvc', 'toaster', '$uibModal', 'jsReportSrvc', 'servicioPropioSrvc', 'authSrvc', 'clienteSrvc', function ($scope, proyectoSrvc, empresaSrvc, tipoProyectoSrvc, proyectoAdjuntoSrvc, tipoAdjuntoSrvc, Upload, activoSrvc, proyectoActivoSrvc, $confirm, tipoDocProySrvc, unidadSrvc, tipoLocalSrvc, DTOptionsBuilder, $filter, servicioBasicoSrvc, toaster, $uibModal, jsReportSrvc, servicioPropioSrvc, authSrvc, clienteSrvc) {
        ...
        $scope.printListaProyectos = () => {
            var modalInstance = $uibModal.open({
                animation: true,
                templateUrl: 'modalListaProyectos.html',
                controller: 'ModalListProyectosCtrl',
                windowClass: 'app-modal-window',
                resolve: {
                    empresas:  () => $scope.lasEmpresas,
                    tipos: () => $scope.losTipoProyecto
                }
            });
        ...
        $scope.delProyecto = function (obj) {
            $confirm({
                text: '¿Seguro(a) de eliminar el proyecto? (Esto eliminará también adjuntos, activos y unidades relacionados al proyecto)',
                title: 'Eliminar proyecto ' + obj.nomproyecto, ok: 'Sí', cancel: 'No'
            }).then(function () {
        ...
        $scope.asignarServicios = function () {
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    proyecto: function () { return $scope.elProyecto; }
                }
            });
        ...
        $scope.delProyectoAdjunto = function (id) {
            $confirm({
                ...
                title: 'Eliminar adjunto', ok: 'Sí', cancel: 'No'
            }).then(function () {
        ...
        $scope.delProyectoActivo = function (qActivo) {
            $confirm({
                text: '¿Seguro(a) de eliminar este activo del proyecto?',
                title: 'Eliminar activo ' + qActivo.finca + '-' + qActivo.folio + '-' + qActivo.libro, ok: 'Sí', cancel: 'No'
            }).then(function () {
        ...
        $scope.delUnidadProyecto = function (obj) {
            $confirm({
                text: '¿Seguro(a) de eliminar la unidad ' + obj.nombre + ' del proyecto?',
                title: 'Eliminar unidad ', ok: 'Sí', cancel: 'No'
            }).then(function () {
        ...
        $scope.delServUni = function (obj) {
            $confirm({
                text: '¿Seguro(a) de quitar este servicio de esta unidad?',
                title: 'Quitar servicio de la unidad', ok: 'Sí', cancel: 'No'
            }).then(function () {
    ...
    proyectoctrl.controller('ModalServicioCtrl', ['$scope', '$uibModalInstance', 'proyecto', 'proyectoSrvc', '$filter', 'toaster', '$confirm', 'servicioBasicoSrvc', function ($scope, $uibModalInstance, proyecto, proyectoSrvc, $filter, toaster, $confirm, servicioBasicoSrvc) {
        ...
        $scope.delSrvProy = function (obj) {
            $confirm({
                text: '¿Seguro(a) de desasignar el servicio ' + obj.numidentificacion + ' del proyecto ' + $scope.proyecto.nomproyecto + ' ?',
                title: 'Desasignar servicio', ok: 'Sí', cancel: 'No'
            }).then(function () {
    ...
    proyectoctrl.controller('ModalListProyectosCtrl', ['$scope', '$uibModalInstance', 'empresas', 'tipos', 'jsReportSrvc', function ($scope, $uibModalInstance, empresas, tipos, jsReportSrvc) {
        ...
        $scope.ok = () => {
            ...
            const test = false;
            jsReportSrvc.getPDFReport(test ? 'Sk0qSE1_v' : 'Sk0qSE1_v', $scope.params).then((pdf) => $scope.content = pdf);
        };
