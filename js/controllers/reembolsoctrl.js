    ...
    reembolsoctrl.controller('reembolsoCtrl', [
        ...
        ($scope, reembolsoSrvc, monedaSrvc, authSrvc, empresaSrvc, $route, $confirm, tipoReembolsoSrvc, DTOptionsBuilder, $filter, tipoFacturaSrvc, tipoCompraSrvc, detContSrvc, cuentacSrvc,
            ...
            $scope.delReembolso = function (obj) {
                $confirm({
                    text: '¿Seguro(a) de eliminar este reembolso? (También se eliminará su detalle)',
                    title: 'Eliminar reembolso', ok: 'Sí', cancel: 'No'
                }).then(function () {
            ...
            $scope.delCompra = function (obj) {
                $confirm({
                    text: '¿Seguro(a) de eliminar esta compra? (También se eliminará su detalle contable)',
                    title: 'Eliminar compra', ok: 'Sí', cancel: 'No'
                }).then(function () {
            ...
            $scope.zeroHaber = function (valor) {
                ...
                if (valor > subtot) {
                    toaster.pop({
                        type: 'error', title: 'Error en el monto del debe.',
            ...
            if ($scope.selectedBanco.id != null && $scope.selectedBanco.id != undefined) {
                if ($scope.selectedTipoMov.abreviatura != null && $scope.selectedTipoMov.abreviatura != undefined) {
                    bancoSrvc.checkTranExists(parseInt($scope.selectedBanco.id), $scope.selectedTipoMov.abreviatura, $scope.numerotranban).then(function (e) {
                        ...
                        if (existe) {
                            toaster.pop({
                                type: 'error', title: 'Cheque existente.',
