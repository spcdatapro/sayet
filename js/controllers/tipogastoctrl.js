(function () {
    ...
    tipogastoctrl.controller('tipogastoCtrl', ['$scope', 'tipogastoSrvc', '$confirm', '$uibModal', function ($scope, tipogastoSrvc, $confirm, $uibModal) {
        ...
        $scope.grpBtnTipoGasto = { i: false, p: false, e: false, u: false, c: false, d: false, a: true, usr: false };
        $scope.grpBtnSubTipoGasto = { i: false, p: false, e: false, u: false, c: false, d: false, a: true };
        $scope.sl = { tipogasto: true, subtipogasto: true };

        $scope.confGrpBtn = function (grp, i, u, d, a, e, c, p, usr) {
            var instruccion = "$scope." + grp + ".i = i; $scope." + grp + ".u = u; $scope." + grp + ".d = d; $scope." + grp + ".a = a; $scope." + grp + ".e = e; $scope." + grp + ".c = c; $scope." + grp + ".p = p; $scope." + grp + ".usr = usr;";
            eval(instruccion);
        };
        ...
        $scope.getTipogasto = function (idtipogasto) {
            ...
            tipogastoSrvc.getTipogasto(idtipogasto).then(function (d) {
                ...
                $scope.confGrpBtn('grpBtnTipoGasto', false, false, true, true, true, false, false, true);
        ...
        $scope.verPermisos = gasto => {
            console.log('ahhh');
            $uibModal.open({
                animation: true,
                templateUrl: 'modalUsuarioGast.html',
                controller: 'ModalPermisosGast',
                resolve: {
                    gasto
                }
            }).result.then(obj => {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------
    tipogastoctrl.controller('ModalPermisosGast', ['$scope', '$uibModalInstance', 'gasto', 'tipogastoSrvc', 'authSrvc', 'toaster',
        function ($scope, $uibModalInstance, gasto, tipogastoSrvc, authSrvc, toaster) {
            ...
            function getAsignados(idgasto) {
                tipogastoSrvc.getUsuarios(idgasto).then(d => {
                    $scope.asignados = d;
                    lstUsuarios();
                });
            }
            ...
            $scope.asignarUsuario = function (idusuario, idgasto) {
                tipogastoSrvc.agregarPermiso(idusuario, idgasto).then(d => {
                    toaster.pop({
                        type: d.tipo, title: 'Permisos de gasto',
                        body: d.mensaje, timeout: 10000
                    });
                    $scope.idusuario = undefined;
                    getAsignados(idgasto);
                });
            }

            $scope.quitarUsuario = function (id) {
                tipogastoSrvc.quitarPermiso(id).then(d => {
                    toaster.pop({
                        type: d.tipo, title: 'Permisos de gasto',
                        body: d.mensaje, timeout: 10000
                    });
                    getAsignados(gasto.id);
                });
            }
