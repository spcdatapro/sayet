(function(){
    ...
    lecturaservicioctrl.controller('lecturaServicioCtrl', ['$scope', 'servicioPropioSrvc', '$filter', '$confirm', 'toaster', 'authSrvc', '$uibModal', 'jsReportSrvc', '$window', function($scope, servicioPropioSrvc, $filter, $confirm, toaster, authSrvc, $uibModal, jsReportSrvc, $window){
        ...
        $scope.params = { idusuario: 0, mes: moment().month() + 1, anio: moment().year(), idservicio: '' };
        $scope.usrdata = {};
        $scope.lecturas = [];
        $scope.meses = [
        ...
        authSrvc.getSession().then(function(usrLogged){ $scope.usrdata = usrLogged; $scope.params.idusuario = +$scope.usrdata.uid; });
        ...
        $scope.getLecturas = function(){
            servicioPropioSrvc.getLectura($scope.params.idusuario, $scope.params.mes, $scope.params.anio).then(function(d){
