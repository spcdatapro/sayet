(function () {
    ...
    var controller = angular.module('cpm.indeminizacion', []);

    controller.controller('rptIndeminizacion', ['$scope', 'jsReportSrvc', 'empresaSrvc', 'proyectoSrvc', function ($scope, 
        jsReportSrvc, empresaSrvc, proyectoSrvc) {
            ...
            $scope.getPDF = function (params) {
                ...
                try {
                    jsReportSrvc.getPDFReport('H16VCiAUxg', params).then(function (pdf) {
                        $scope.content = pdf;
                        $scope.cargando = false;
                    })
                } catch (err) {
            ...
            $scope.getXML = function (params) {
                ...
                try {
                    jsReportSrvc.getReport('BJhM6IXObx', params).then(function (result) {
                        var file = new Blob([result.data], { type: 'application/vnd.ms-excel' });
                        let rango = params.anio;

                        saveAs(file, 'Reporte_Indeminizacion_' + rango + '.xlsx');
